...
#define AnisoModeNames "Const" | "1/R" | "Border" 
    ...
    public:
        ...
        bool get_fg(Delaunay::Vertex_handle v, double& f, vec2& grad_f) ;  //dxy modified
        double lloyd_energy() ;
	...
	void load_affinity(const std::string& filename) ;
	void funcgrad_affinity(const double* x, double& f, double* g) ;
	GLfloat& affinity_weight() { return affinity_weight_ ; }
	bool has_affinity() const { return has_affinity_ ; }
	double affinity(unsigned int i, unsigned int j) {
	    gx_debug_assert(has_affinity()) ;
	    gx_debug_assert(i < 256) ;
	    gx_debug_assert(j < 256) ;
	    return affinity_[i][j] ;
	}

	///dxy add: core
	void get_edge_direction_fg(const vec2& edge, const vec2& v, double& f, vec2& grad_f); 
	...
	//
	void get_field(const vec2& v, double& angle, vec2& grad);
	///
	...
	GLboolean&  use_true_grad()  { return use_true_grad_;  }
    ...
    private:
		...
		GLboolean use_true_grad_; //dxy test
	...
	GLfloat affinity_weight_ ;
	bool has_affinity_ ;
	double affinity_[256][256] ;
    } ;
